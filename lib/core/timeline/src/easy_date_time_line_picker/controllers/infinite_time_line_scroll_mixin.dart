import 'package:find_my_doc/core/timeline/src/easy_date_time_line_picker/utils/date_time_extensions.dart';
...
mixin InfiniteTimeLineScrollMixin on ScrollController {
  ...
  String _generateCacheKey(DateTime first, DateTime last, double itemExtent) => '${first.millisecondsSinceEpoch}-${last.millisecondsSinceEpoch}-$itemExtent';
  ...
  int _calculateDaysDifference(DateTime first, DateTime last) {
    ...
    final differencesInHours = normalizedLast.difference(normalizedFirst).inHours;
  ...
  double getScrollOffsetFromSelectionMode({
    ...
    return switch (selectionMode) {
      SelectionModeNone() || SelectionModeAlwaysFirst() => _calculateFirstPosition(
          itemExtent: itemExtent,
          firstDate: firstDate,
          lastDate: lastDate,
