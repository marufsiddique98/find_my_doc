...
class _MonthYearPickerState extends State<MonthYearPicker> {
  ...
  void _handleYearChanged(DateTime date) {
    ...
    if (date.isBefore(widget.firstDate)) {
      ...
      if (date.isAfter(widget.lastDate)) {
        date = DateTime(date.year - 1, widget.lastDate.month, widget.lastDate.day);
      }
    }
  ...
  DateTime? _findFirstActiveDayInMonth(DateTime date) {
    return generateMonthDays(date).firstWhereOrNull(
      (date) => !widget.disableStrategy.isDisabled(date) && _isDateInRange(date),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String effectiveTitle = DateFormat(_isYearDisabled(_selectedYear) ? DateFormatUtils.yearNumber : DateFormatUtils.monthY, widget._locale).format(_selectedYear);
    ...
    final String effectiveCancelText = widget.cancelText ?? ((useMaterial3 ? localizations?.cancelButtonLabel : localizations?.cancelButtonLabel.toUpperCase()) ?? cancelButtonLabel);
    final String effectiveConfirmText = widget.confirmText ?? (localizations?.okButtonLabel ?? okButtonLabel);
      ...
      child: Padding(
        ...
        child: Align(
          ...
          child: OverflowBar(
            spacing: 8,
            children: <Widget>[
              widget.cancelButtonBuilder?.call(context, _handleCancel, effectiveCancelText) ??
                  TextButton(
              ...
              widget.confirmButtonBuilder?.call(context, _handleOk, effectiveConfirmText) ??
                  TextButton(
    ...
    return Dialog(
      ...
      elevation: useMaterial3 ? datePickerTheme.elevation ?? defaults.elevation! : datePickerTheme.elevation ?? 24,
      ...
      shape: useMaterial3 ? datePickerTheme.shape ?? defaults.shape : datePickerTheme.shape ?? defaults.shape,
